...
notificationIcon.addEventListener("click", () => {
  if (!isNotificationVisible) {
    // Hi?n th? thông báo n?u nó không hi?n th?
    for(var i =0;i<notification.length;i++ ){
        notification[i].style.display = "block"; // ho?c s? d?ng các l?p CSS
        notification[i].style.top =i*6+"0px";
    }
    
    isNotificationVisible = true;
  } else {
    // ?n thông báo n?u nó ?ang hi?n th?
    for(var i =0;i<notification.length;i++ ){
    notification[i].style.display = "none"; 
    }
    //// ho?c s? d?ng các l?p CSS
...
<style>
 #notification {
    ...
    width: 320px; /* ?i?u ch?nh kích th??c theo mong mu?n */
...
.notification-time {
    margin-top: -10px; /* Di chuy?n lên trên 10px */
...
#notificationIcon {
  font-size: 24px; /* Thay ??i kích th??c bi?u t??ng theo mong mu?n */
}
...
#notificationCount {
  ...
  top: 0;
  right: 0;
  background-color: red; /* Màu n?n cho s? thông báo */
  color: white; /* Màu ch? cho s? thông báo */
  border-radius: 50%; /* Kích th??c góc bo tròn */
  padding: 4px 8px; /* Kho?ng cách gi?a s? và bi?u t??ng */
  font-size: 12px; /* Kích th??c c?a s? thông báo */
}
